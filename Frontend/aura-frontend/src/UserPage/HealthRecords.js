...
const Diseases = () => {
  ...
  const handleAddDisease = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/user/update-patient`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  const handleStatusChange = async (diseaseId, newStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/api/user/update-med-status`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  if (loading) return <div className="profile-content"><p>Loading diseases...</p></div>;
